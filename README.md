...
Below, we provide a demo for this particle localization algorithm. We recorded this demo in two ways: one view includes both the robot's movement in the maze and Rviz view; the other only recorded Rviz view directly from computer with higher clarity. We also recorded a bag with the following topics: `/map`, `/scan`, `/cmd_vel`, `/particle_cloud`, `/estimated_robot_pose` and [click here](https://github.com/lwu5/particle_filter_project/tree/main/bags) for the bag's location.

Note: Please discard the virtual turtlebot in the Rviz, it does not correctly reflect on the robot's position and movement in reality. Please refer to the demo video for the actual robot position and movement.
...
- **Code Description**: We simply iterate through every particle in the cloud to calculate the weighted average in terms of x, y, and yaw and convert yaw to quaternion before assign them to robot's estimated pose. 
...
- Some areas of the maze are very similar, which confuses the algorithm. We definitely realized our algorithm performs better in those areas where configurations of walls are more unique.

## Future Work

- If we had more time, we would want to improve the localization so that it converges faster. We were generally limited in how many particles we could include in the model because we got the error that rviz’s time goes backwards if we included too many. Therefore we would like to be able to increase the number of particles to see how well it can perform with more of them included. Also we would want to spend more time further optimizing parameters like noise and the standard deviations for likelihood so that the robot can be accurately localized in space given this larger particle amount. It would also be cool to be able to obtain different maps and test the localization ability in different environments that are not so narrow or similar like the maze. Sometimes after converging our cloud would disperse slightly but ultimately do well in converging back to the robot’s position; this showed us that our particle filter performs well in converging back despite potential noisy measurements / similarities in maze distances but we would like to see how this convergence performs across different environments to further refine the particle cloud algorithm. 

## Takeaways

1. One key takeaway was learning about probabilistic sampling and trial and error with corresponding parameters that will yield the optimal result. We gained a lot of intuition in thinking about how changing one parameter might increase the ability of different particle clouds to cluster more broadly, converge on single locations more quickly, and other types of results -- basically linking parameter changes to expected behaviors and going through that thought process was really helpful. We also gained experience in systematic, meticulous testing of more types of variables to hone in on what elements are the source of our improvements/decreases in performance. This will help us in future projects where we have to optimize more types of parameters and reason through why changing those parameters led to changes in behavior. 
2. Another key takeaway from this project was learning how to manipulate variables in different coordinate systems and thinking about how to transform between maps vs. real-life spaces. Managing all the different conversions from the data sent from the robot and testing how they are oriented relative to the RViz environment gave us a lot of experience in thinking about robot control in these transformed ways; this will allow us to perform more complex implementations of robot behavior when we are not just manipulating it based on how we observe it moves in real life/in a virtual environment (which was like the level of proficiency we got in the warmup project), but also how movement can be simulated for even “fake robots” (e.g. particles) within a map so that we can use more data to train an algorithm to converge on some desired goal (a robot location, a learned behavior, etc.).
